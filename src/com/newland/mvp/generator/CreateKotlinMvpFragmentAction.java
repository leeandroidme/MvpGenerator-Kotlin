...
public class CreateKotlinMvpFragmentAction extends AnAction {
    ...
    private void create(final PsiDirectory directory, final String name, final Module module) {
        ...
        this.createPsiClass(directory, fragmentContractName, fileTemplateManager, FileTemplateProvider.MVP_FRAGMENT_CONTRACT, new HashMap<String, String>() {
            {
                this.put("FRAGMENT_NAME", name);
                this.putAll(map);
            }
        });
        this.createPsiClass(directory, fragmentModuleName, fileTemplateManager, FileTemplateProvider.MVP_FRAGMENT_MODULE, new HashMap<String, String>() {
            {
                this.put("FRAGMENT_NAME", name);
                this.putAll(map);
            }
        });
        this.createPsiClass(directory, fragmentPresenterName, fileTemplateManager, FileTemplateProvider.MVP_FRAGMENT_PRESENTER, new HashMap<String, String>() {
            {
                this.put("FRAGMENT_NAME", name);
                this.putAll(map);
            }
        });
        this.createPsiClass(directory, fragmentSubComponentName, fileTemplateManager, FileTemplateProvider.MVP_FRAGMENT_SUB_COMPONENT, new HashMap<String, String>() {
            {
                this.put("FRAGMENT_NAME", name);
                this.putAll(map);
                ...
                try {
                    bindMethodStr = fileTemplateManager.getJ2eeTemplate(FileTemplateProvider.MVP_FRAGMENTBINDINJECTMETHOD).getText(properties);
                } catch (Exception e) {
                    e.printStackTrace();
                }
